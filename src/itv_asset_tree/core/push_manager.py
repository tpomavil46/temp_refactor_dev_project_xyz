...
class PushManager:
    ...
    def push(self, metadata_state_file=None):
        """
        Pushes the asset tree to Seeq, ensuring the workbook exists before pushing.
        """
        try:
            print(f"🚀 [DEBUG] About to push tree '{self.tree.name}' to Seeq...")
            ...
            workbook_name = self.tree._workbook
            print(f"📂 [DEBUG] Ensuring workbook '{workbook_name}' exists...")

            workbooks = spy.workbooks.search(query={"Name": workbook_name})  

            if workbooks.empty:
                print(f"📂 [INFO] Workbook '{workbook_name}' not found. Creating it as an Analysis...")

                # ✅ Correct `spy.workbooks.push()` usage
                spy.workbooks.push(name=workbook_name, workbook_type="Analysis")

            # **Use `spy.push()` for metadata**
            print(f"📊 [DEBUG] Converting tree to DataFrame before push...")
            tree_df = self.tree.to_frame()
            print(f"📊 [DEBUG] Tree DataFrame before push:\n{tree_df}")

            result = spy.push(metadata=tree_df, workbook=workbook_name)

            print(f"✅ [DEBUG] Tree '{self.tree.name}' successfully pushed via `spy.push()`.")
            return {"message": f"Tree '{self.tree.name}' pushed successfully.", "result": result}
