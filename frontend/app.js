...
function attachPushTreeListener() {
    ...
    if (!pushTreeButton) {
        console.error("❌ Push Tree button not found in DOM!");
        return;
    }
    ...
    pushTreeButton.addEventListener("click", async () => {
        ...
        try {
            ...
            // ✅ EXPLICITLY CALL updateTreeVisualization()
            console.log("📡 Updating tree visualization now...");
...
async function updateTreeVisualization(treeName, workbookName) {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8000/visualize_tree/?tree_name=${treeName}&workbook_name=${workbookName}`);
        ...
        console.log("✅ Tree visualization updated successfully!");
    } catch (error) {
        console.error("❌ Error updating tree visualization:", error);
        alert("Failed to update tree visualization.");
    }
}
